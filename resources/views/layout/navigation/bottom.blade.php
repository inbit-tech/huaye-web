<section class="hero is-light">
	<div class="hero-body">
		<div class="container">
			<nav class="level container">
				<div class="level-item has-text-centered">
					<div class="has-text-left has-text-centered-touch">
						<p class="has-padding-btm-18"><a href="/products" class="use-noto-900 is-uppercase is-size-7 is-size-6-touch">@lang('navi.products-bottom')</a></p>
						<p><a href="/bec">@lang('navi.prod1')</a></p>
						<p><a href="/uht">@lang('navi.prod2')</a></p>
						<p><a href="/litz">@lang('navi.prod3')</a></p>
					</div>
				</div>
				<div class="level-item has-text-centered">
					<div class="has-text-left has-text-centered-touch">
						<p class="has-padding-btm-18"><a href="/about" class="use-noto-900 is-uppercase is-size-7 is-size-6-touch">@lang('navi.about')</a></p>
						<p><a href="/about#our-service">@lang('navi.service')</a></p>
						<p><a href="/about#our-quality">@lang('navi.quality')</a></p>
						<p><a href="/about#our-story">@lang('navi.intro')</a></p>
					</div>
				</div>
				<div class="level-item has-text-centered">
					<div class="has-text-left has-text-centered-touch">
						<p class="has-padding-btm-18"><a href="/contact" class="use-noto-900 is-uppercase is-size-7 is-size-6-touch">@lang('navi.contact')</a></p>
						<p><a href="tel:0769-82236800">@lang('navi.phone1')</a></p>
						<p><a href="tel:0769-82601699">@lang('navi.phone2')</a></p>
						<p><a href="mailto:huaye@huaye-dg.com">@lang('navi.mail')</a></p>
					</div>
				</div>
				<div class="level-item has-text-centered">
					<div class="has-text-left has-text-centered-touch">
						<p class="use-noto-900 is-uppercase is-size-7 is-size-6-touch has-padding-btm-18">@lang('navi.lang')</p>
						<p><a href="lang/en">English</a></p>
						<p><a href="lang/zh">中文</a></p>
					</div>
				</div>
			</nav>
		</div>
	</div>
</section>


<footer class="footer">
	<div class="content has-text-centered has-text-white is-size-7 use-noto-500">
		<p>2019 @lang('footer.copyright')</p>
		<p><a href="http://www.beian.miit.gov.cn" class="has-text-white">粤ICP备19048719号</a></p>
	</div>
</footer>